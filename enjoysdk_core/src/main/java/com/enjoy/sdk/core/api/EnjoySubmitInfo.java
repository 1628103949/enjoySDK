...
/**
 * Description: enjoy sdk submit info.
 * Data：25/09/2018-4:11 PM
 * Author: ranger
 */
public class EnjoySubmitInfo {
    ...
    boolean checkParam() {
        ...
        if (TextUtils.isEmpty(submitType)) {
            ...
            if (!submitType.equals(EnjoyConstants.SUBMIT_TYPE_CREATE) &&
                    !submitType.equals(EnjoyConstants.SUBMIT_TYPE_ENTER) &&
                    !submitType.equals(EnjoyConstants.SUBMIT_TYPE_UPGRADE) &&
                    !submitType.equals(EnjoyConstants.SUBMIT_TYPE_UPDATE)) {
                checkOK = false;
            }

            if (TextUtils.isEmpty(submitLastRoleName) && submitType.equals(EnjoyConstants.SUBMIT_TYPE_UPDATE)) {
                checkOK = false;
            }
        }
        return checkOK;
    ...
    void debugParam(Activity act, ConfirmDialog.ConfirmCallback callback) {
        ...
        if (submitTimeUp == -1 && submitType != null && submitType.equals(EnjoyConstants.SUBMIT_TYPE_UPGRADE)) {
        ...
        if ( EnjoyApplication.getDebugConfig() && (!mustParams.isEmpty() || !optionalParams.isEmpty())) {
    ...
    public HashMap<String, String> toHash() {
        ...
        info.put(EnjoyConstants.SUBMIT_ROLE_ID, submitRoleId);
        info.put(EnjoyConstants.SUBMIT_ROLE_NAME, submitRoleName);
        info.put(EnjoyConstants.SUBMIT_ROLE_LEVEL, submitRoleLevel + "");
        info.put(EnjoyConstants.SUBMIT_SERVER_ID, submitServerId);
        info.put(EnjoyConstants.SUBMIT_SERVER_NAME, submitServerName);
        info.put(EnjoyConstants.SUBMIT_BALANCE, submitBalance + "");
        info.put(EnjoyConstants.SUBMIT_VIP, submitVip + "");
        info.put(EnjoyConstants.SUBMIT_PARTY_NAME, submitParty);
        info.put(EnjoyConstants.SUBMIT_EXT, submitExt);
        info.put(EnjoyConstants.SUBMIT_CREATE_TIME, String.valueOf(submitTimeCreate));
        info.put(EnjoyConstants.SUBMIT_UP_TIME, String.valueOf(submitTimeUp));
        info.put(EnjoyConstants.SUBMIT_LAST_ROLE_NAME, submitLastRoleName);
        info.put(EnjoyConstants.SUBMIT_TYPE, submitType);
        return info;
    }
    ...
    public String toString() {
        try {
            ...
            data.put(EnjoyConstants.SUBMIT_ROLE_ID, submitRoleId);
            data.put(EnjoyConstants.SUBMIT_ROLE_NAME, submitRoleName);
            data.put(EnjoyConstants.SUBMIT_ROLE_LEVEL, submitRoleLevel);
            data.put(EnjoyConstants.SUBMIT_SERVER_ID, submitServerId);
            data.put(EnjoyConstants.SUBMIT_SERVER_NAME, submitServerName);
            data.put(EnjoyConstants.SUBMIT_BALANCE, submitBalance);
            data.put(EnjoyConstants.SUBMIT_VIP, submitVip);
            data.put(EnjoyConstants.SUBMIT_PARTY_NAME, submitParty);
            data.put(EnjoyConstants.SUBMIT_EXT, submitExt);
            data.put(EnjoyConstants.SUBMIT_CREATE_TIME, submitTimeCreate);
            data.put(EnjoyConstants.SUBMIT_UP_TIME, submitTimeUp);
            data.put(EnjoyConstants.SUBMIT_LAST_ROLE_NAME, submitLastRoleName);
            data.put(EnjoyConstants.SUBMIT_TYPE, submitType);
    ...
    public static class SubmitBuilder {
        ...
        public EnjoySubmitInfo build() {
            EnjoySubmitInfo submitInfo = new EnjoySubmitInfo();
