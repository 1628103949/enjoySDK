...
public class AccountRegView extends LoginBaseView<AccountRegView> {
    ...
    protected View createContentView() {
        View containView = LayoutInflater.from(mContext).inflate(ResUtil.getLayoutID("enjoy_reg_view", mContext), null);
    ...
    private void doRegister() {
        ...
        x.http().post(userRegParam, new Callback.CommonCallback<EnjoyResponse>() {
            @Override
            public void onSuccess(final EnjoyResponse result) {
                onViewFinishLoad();
                if (result.state == 0) {
                    //服务端返回数据为空
                    ViewUtils.sdkShowTips(mActivity, "注册失败："+result.msg);
                }else{
                    ...
                    showSuccessDialog(new RegSuccessDialog.SuccessCallback() {
                        ...
                        public void onFinish() {
                            mLoginDialog.getEnjoyAccount().dealLoginSuccResult(EnjoyAccount.ACCOUNT_REG, result, currentPwd);
                        }
                    });
                }
            ...
            public void onError(Throwable ex, boolean isOnCallback) {
                ...
                if (isAutoRandom) {
                    ...
                    mLoginDialog.getEnjoyAccount().dealLoginFailResult(EnjoyAccount.ACCOUNT_REG, ex);
                }
