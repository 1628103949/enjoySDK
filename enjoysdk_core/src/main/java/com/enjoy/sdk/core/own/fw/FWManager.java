...
import com.enjoy.sdk.core.own.fw.view.BallMenuView;
import com.enjoy.sdk.core.own.fw.view.BallRootView;
import com.enjoy.sdk.core.own.fw.view.BallView;
import com.enjoy.sdk.core.own.fw.view.listener.BallClickListener;
import com.enjoy.sdk.core.own.fw.view.listener.BallTouchListener;
...
import com.enjoy.sdk.framework.view.dialog.Attention.ShakeVertical;
...
//        }
    ...
    private void initHideView() {
        ...
        hideView = LayoutInflater.from(mActivity).inflate(ResUtil.getLayoutID("enjoy_float_hide_view", mActivity), null);
    ...
    public void handleTNBallEv(EnjoyBallEv tnBallEv) {
        switch (tnBallEv.getAction()) {
            case EnjoyBallEv.TO_SHOW:
                show();
                break;
            case EnjoyBallEv.TO_HIDE:
                hide();
                break;
            default:
                break;
        ...
        if (toShow) {
            if (ViewCompat.isAttachedToWindow(hideView)) {
                if (isHideActive(screenX, screenY)) {
                    //激活隐藏状态
                    hideCloseImg.setImageResource(ResUtil.getDrawableID("enjoy_hide_closing", mActivity));
                    hideCloseTv.setTextColor(Color.parseColor("#EC6B62"));
                } else {
                    //不激活隐藏
                    hideCloseImg.setImageResource(ResUtil.getDrawableID("enjoy_hide_close", mActivity));
