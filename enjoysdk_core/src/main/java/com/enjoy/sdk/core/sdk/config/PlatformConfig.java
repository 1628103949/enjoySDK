...
public class PlatformConfig {
    private static final String CONFIG_FILE_NAME = "enjoy" + File.separator + "enjoy_platform.ini";
    ...
    private PlatformConfig(Context ctx) {
        ...
        if (properties != null) {
            ...
            if (properties.containsKey("pAppId")) {
                pAppId = EnjoyEncrypt.decryptInfo(properties.getProperty("pAppId"));
            }
            if (properties.containsKey("pAppKey")) {
                pAppKey = EnjoyEncrypt.decryptInfo(properties.getProperty("pAppKey"));
            }
            if (properties.containsKey("pPayId")) {
                pPayId = EnjoyEncrypt.decryptInfo(properties.getProperty("pPayId"));
            }
            if (properties.containsKey("pPayKey")) {
                pPayKey = EnjoyEncrypt.decryptInfo(properties.getProperty("pPayKey"));
            }
        ...
        if (TextUtils.isEmpty(platformClass)) {
            //默认platform
            platformClass = "com.enjoy.sdk.core.open.OPlatformSDK";
        }

        if (TextUtils.isEmpty(appClass)) {
            //默认app
            appClass = "com.enjoy.sdk.core.open.OPlatformAPP";
        }
