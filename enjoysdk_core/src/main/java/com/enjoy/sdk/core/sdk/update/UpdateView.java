package com.enjoy.sdk.core.sdk.update;
...
import com.enjoy.sdk.framework.xutils.common.task.PriorityExecutor;
...
public class UpdateView extends BaseDialog<UpdateView> {
    ...
    public View onCreateView() {
        View containerView = LayoutInflater.from(mContext).inflate(ResUtil.getLayoutID("enjoy_update_dialog", mContext), null);
    ...
    private void checkDownload() {
        ...
        if (!netType.equals("WIFI")) {
            //非WIFI情况，弹出确认下载弹窗
            if (isForce) {
                ViewUtils.showTipsConfirm(mContext, mContext.getString(ResUtil.getStringID("enjoy_not_wifi_type_download_tips", mContext)), new TipsDialog.TipsConfirmCallback() {
                    @Override
                ...
                ViewUtils.showTipsDialog(mContext, true, mContext.getString(ResUtil.getStringID("enjoy_not_wifi_type_download_tips", mContext)), new TipsDialog.TipsCallback() {
                    @Override
    ...
    private void doDownload(boolean cache) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            currentApkPath = mContext.getCacheDir() + File.separator + "enjoy_update_files" + File.separator + TNUpdate.getFileNameFormUrl(mDownloadUrl);
        } else {
            currentApkPath = mContext.getExternalCacheDir() + File.separator + "enjoy_update_files" + File.separator + TNUpdate.getFileNameFormUrl(mDownloadUrl);
        }
    ...
    private class DownloadCallback implements
        ...
        public void onSuccess(final File result) {
            ...
            //下载完成后，提示安装
            if (isForce) {
                ViewUtils.showTipsConfirm(mContext, mContext.getString(ResUtil.getStringID("enjoy_update_to_install", mContext)), new TipsDialog.TipsConfirmCallback() {
                    @Override
                    public void onConfirm() {
                        //安装
                        try {
                ...
                ViewUtils.showTipsDialog(mContext, true, mContext.getString(ResUtil.getStringID("enjoy_update_to_install", mContext)), new TipsDialog.TipsCallback() {
                    @Override
                    public void onCancel() {
                        UpdateView.this.dismiss();
                    }
        ...
        public void onError(Throwable ex, boolean isOnCallback) {
            //下载错误，提示重试
            if (isForce) {
                ViewUtils.showTipsConfirm(mContext, mContext.getString(ResUtil.getStringID("enjoy_update_to_retry", mContext)), new TipsDialog.TipsConfirmCallback() {
                    @Override
                    public void onConfirm() {
                        useCache = false;
                        checkDownload();
                ...
                ViewUtils.showTipsDialog(mContext, true, mContext.getString(ResUtil.getStringID("enjoy_update_to_retry", mContext)), new TipsDialog.TipsCallback() {
                    @Override
                    public void onCancel() {
                        UpdateView.this.dismiss();
                    }
