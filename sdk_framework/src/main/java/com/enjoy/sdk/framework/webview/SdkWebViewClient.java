...
public class SdkWebViewClient extends WebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
        ...
        //微信H5支付处理
        if (url.startsWith(EnjoyEncrypt.decryptInfo("vkNPzIHFu5sGidyBU2BBXnPaT5WnDqSAnzgJstvmaeU"))) {
        ...
        if (url.startsWith(EnjoyEncrypt.decryptInfo("2G+io4ppibBGDqQUE7vW+Q==")) || (url.startsWith("intent") && url.contains(EnjoyEncrypt.decryptInfo("2G+io4ppibBGDqQUE7vW+Q==")))) {
            Intent intent = new Intent();
            intent.setAction(Intent.ACTION_VIEW);
            intent.setData(Uri.parse(url));
            if (checkApkExist(mContext, intent)) {
